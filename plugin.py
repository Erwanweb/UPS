...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if len(UPSReadedValues) == 4:
            ...
            # self.TIMEMIN = round(((3350 / ((self.POWER / 10.1) / 0.8)) * (self.PERCENT / 100)) * 60) # For UPS 3S
            # For UPS(C)
            if self.VOLTAGE >= 7900:
                self.TIMEMIN = round(((((3350 / (0 - self.CURRENT) ) * 0.8) * (self.PERCENT / 100)) * 60) * 1)
            elif self.VOLTAGE >= 7500:
                self.TIMEMIN = round(((((3350 / (0 - self.CURRENT) ) * 0.8) * (self.PERCENT / 100)) * 60) * 0.9)
            elif self.VOLTAGE >= 7000:
                self.TIMEMIN = round(((((3350 / (0 - self.CURRENT) ) * 0.8) * (self.PERCENT / 100)) * 60) * 0.8)
            else :
                self.TIMEMIN = round(((((3350 / (0 - self.CURRENT)) * 0.8) * (self.PERCENT / 100)) * 60) * 0.7)
            self.TIMESEC = self.TIMEMIN * 60
            if self.CURRENT >= -150: #power supply on
