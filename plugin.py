...
"""
"""
<plugin key="AC-UPS" name="AC UPS Monitoring Plugin" author="Erwanweb" version="1.1.1" externallink="https://github.com/Erwanweb/UPS.git">
    <description>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Domoticz.Device(Name="Power supply", Unit=2, TypeName="Switch", Image=9, Used=1).Create()
            devicecreated.append(deviceparam(1, 0, ""))  # default is Off
        if 2 not in Devices:
            Domoticz.Device(Name="Battery charging", Unit=2, TypeName="Switch", Image=9, Used=1).Create()
            devicecreated.append(deviceparam(2, 0, ""))  # default is Off
        if 3 not in Devices:
            Domoticz.Device(Name="Battery Level", Unit=4, Type=243, Subtype=6, Image=9, Used=1).Create()
    ...
    def onHeartbeat(self):
        ...
        cmd = 'sudo python3 ./readups.py short'
        output = sp.getoutput(cmd)
        Domoticz.Debug("UPS Values are : {}".format(output))
        UPSReadedValues = parseCSV(output)
        if len(UPSReadedValues) == 3:
            self.VOLTAGE = CheckParam("UPS Voltage", UPSReadedValues[0], 0)
            self.CURRENT = CheckParam("UPS Current", UPSReadedValues[1], 0)
            self.PERCENT = CheckParam("UPS Battery Level", UPSReadedValues[2], 0)
        else:
            Domoticz.Error("Error reading UPS Values")
        os.system(cmd)
