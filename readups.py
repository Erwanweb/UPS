...
def read(ina219, parameter):
    ...
    # fos UPS(C)
    if (voltage_batt >= 7900):
        timemin = round(((((3350 / (0 - current)) * 0.8) * (percent / 100)) * 60) * 1)
    elif (voltage_batt >= 7500):
        timemin = round(((((3350 / (0 - current)) * 0.8) * (percent / 100)) * 60) * 0.9)
    elif (voltage_batt >= 7000):
        timemin = round(((((3350 / (0 - current)) * 0.8) * (percent / 100)) * 60) * 0.8)
    else:
        timemin = round(((((3350 / (0 - current)) * 0.8) * (percent / 100)) * 60) * 0.7)
    # timemin = (((3350 / (0 - current) ) * 0.8) * (percent / 100)) * 60
    result = ""
    if(PowerFormatting.FULL == parameter):
        result = printlong(voltage, shunt_voltage, current, power, percent, timemin)
...
def printlong(voltage, shunt_voltage, current, power, percent, timemin):
    discharge = ""
    if(current < -100):
        discharge = "dis"
    print("--------------------------------------------------------------")
    print("Load Voltage:\t\t{:6.3f} V".format(voltage))
    print("Shunt Voltage:\t\t{:6.3f} V".format(shunt_voltage))
