...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if len(UPSReadedValues) == 4:
            ...
            #self.RemainTime['Hour'] = time.strftime("%H", time.gmtime(LastCalcTime))
            #self.RemainTime['Minute'] = time.strftime("%M", time.gmtime(LastCalcTime))
            #HourofTR = self.RemainTime['Hour']
            #MinofTR = self.RemainTime['Minute']
            HourofTR = time.strftime("%H", time.gmtime(LastCalcTime))
            MinofTR = time.strftime("%M", time.gmtime(LastCalcTime))
            Domoticz.Debug("Time remaining on Batt. is : {} H {} Mins".format(HourofTR, MinofTR))
        else:
        ...
        if self.CURRENT >= -150:
            ...
            #LastRemainTime = self.TIMEMIN * 60
            Devices[2].Update(nValue=4, sValue="Power supply DOWN - Batt. at {} %".format(self.BatteryLevel))
            if self.BatteryLevel > 0:
