...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if len(UPSReadedValues) == 4:
            ...
            #LastCalcTime = self.TIMESEC
            #HourofTR = time.strftime("%H", time.gmtime(LastCalcTime))
            #MinofTR = time.strftime("%M", time.gmtime(LastCalcTime))
            #Domoticz.Debug("Remaining time if working on Battery is : {} H {} Mins".format(HourofTR, MinofTR))
        else:
        ...
        if self.CURRENT >= -150:
            ...
            if (Devices[1].nValue == 1): #Power supply just done, so we change the DZ Widget and calculate the remaining time on battery.
                ...
                Domoticz.Debug("Power supply just done- Rem. time on Battery is : {} H {} Mins".format(self.HourofTR, self.MinofTR))
            self.PowerSupply = False
            Domoticz.Debug("Battery Dicharging")
            """
            if self.LastBatteryLevel > self.BatteryLevel :
                if self.TIMESEC > 59 :
                    Devices[2].Update(nValue=4, sValue="Power supply DOWN - Batt. at {} % - Rem. Time : {} H {} Mins".format(self.BatteryLevel, HourofTR, MinofTR))
                else :
                    Devices[2].Update(nValue=4, sValue="Power supply DOWN - Batt. at {} % - Rem. Time : {} Mins".format(self.BatteryLevel, MinofTR))
                if self.BatteryLevel > 0:
                    Domoticz.Log("Power supply is DOWN, Battery is discharging, Battery Level is now at {} %".format(self.BatteryLevel))
                else:
                    Domoticz.Log("Power supply is DOWN, Battery is discharging, Battery Level is undefined - Please wait for calculation")
                if self.BatteryLevel > 0 :
                    self.LastBatteryLevel = self.BatteryLevel
            """
            if self.BatteryLevel > 0:
                ...
                if self.LastBatteryLevel > self.BatteryLevel : #or (self.LastBatteryLevel = 0) :
                    ...
                    if self.CalcTime > self.TIMESEC : # or self.CalcTime = 0 :
                        ...
                        LastCalcTime = self.CalcTime
                        self.HourofTR = time.strftime("%H", time.gmtime(self.CalcTime))
                        self.MinofTR = time.strftime("%M", time.gmtime(LastCalcTime))
                        Domoticz.Debug("Remaining time if working on Battery is : {} H {} Mins".format(HourofTR, MinofTR))
                    if self.CalcTime > 59 :
                        Devices[2].Update(nValue=4, sValue="Power supply DOWN -Batt.:{} % -R.Time: {} H {} Mins".format(self.BatteryLevel, self.HourofTR, self.MinofTR))
                    else :
                        Devices[2].Update(nValue=4, sValue="Power supply DOWN -Batt.:{} % -R.Time: {} Mins".format(self.BatteryLevel, self.MinofTR))
            else:
                Domoticz.Log("Power supply is DOWN, Battery is discharging, Battery Level is undefined - Please wait for calculation")
                Devices[2].Update(nValue=4, sValue="Power supply DOWN - undef. Batt. Level, wait for calculation")
                #self.LastBatteryLevel = 0
